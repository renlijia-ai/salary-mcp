...
// 定义合并单元格内容映射类型
interface MergedContentMap {
  [key: string]: { content: string };
}

// 将工作表转为带合并的HTML表格
// 获取单元格内容，包括公式和值
function getExcelCellContent(excelCell: ExcelCell | undefined): string {
  if (!excelCell) return "";
  // 如果有公式，显示公式文本
  if (excelCell.f) {
    return `=${excelCell.f}`;
...
function isExcelRowEmpty(
  ...
  for (let colIndex = sheetRange.s.c; colIndex <= sheetRange.e.c; colIndex++) {
    ...
    const currentCell = worksheet[cellAddress] as ExcelCell | undefined;
    if (
      currentCell &&
...
function convertExcelSheetToHtml(
  worksheet: XLSX.WorkSheet,
  sheetName: string
): string {
  ...
  // 获取所有行和列范围
  const sheetRange = XLSX.utils.decode_range(
    excelSheet["!ref"] || "A1"
  ) as SheetRange;
  ...
  // 合并信息
  const mergedCells = (excelSheet["!merges"] || []) as MergedCell[];

  // 构建合并单元格内容映射表
  const mergedContentMap: MergedContentMap = {};
  mergedCells.forEach((mergedCell) => {
    const { s: start, e: end } = mergedCell;
    // 获取主单元格的内容和样式
    const mainCellAddress = XLSX.utils.encode_cell({ r: start.r, c: start.c });
    const mainCell = excelSheet[mainCellAddress] as ExcelCell | undefined;
  ...
  for (let rowIndex = sheetRange.s.r; rowIndex <= sheetRange.e.r; rowIndex++) {
    ...
    for (
      let colIndex = sheetRange.s.c;
      colIndex <= sheetRange.e.c;
      colIndex++
    ) {
      // 跳过空列
      if (isExcelColumnEmpty(excelSheet, colIndex, sheetRange)) {
      ...
      if (mergedContent) {
        ...
        const currentCell = excelSheet[cellAddress] as ExcelCell | undefined;
...
export async function convertExcelUrlToHtml(
  excelUrl: string
